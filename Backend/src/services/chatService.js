 ...
 */
const interviewChat = async (message, history, problemTitle, problemDescription, code, language, cvFileName, user) => {
    if (user.tokens <= 0) {
        throw new Error("INSUFFICIENT_TOKENS");
    }
    ...
    if (history && history.length > 0) {
        conversation = history.map(h => `${h.role === 'ai' ? 'Interviewer' : 'Candidate'}: ${h.text}`).join('\n');
    }

    const engineeredPrompt = `The candidate is in a live mock coding interview for a software engineering role.
You are the AI interviewer. Speak to them conversationally, ask technical questions, critique their answers, offer guidance if they get stuck, and guide them through solving the problem.

Problem Details:
Title: ${problemTitle || 'Unknown'}
Description: ${problemDescription || 'Unknown'}

Candidate's Current Code (${language || 'javascript'}):
\`\`\`${language || 'javascript'}
${code || '// no code written yet'}
\`\`\`

Candidate's Resume/CV (if uploaded): ${cvFileName || 'None'}
...
AI Interviewer Response (Make it conversational, direct, and under 3-4 sentences so it is comfortable for speech synthesis):`;

    const systemInstruction = `You are a professional software engineering interviewer. 
1. Maintain a friendly yet professional tone.
2. If the candidate uploaded a resume, occasionally tailor questions to their resume experience.
3. Be conversational and concise. Keep your spoken responses under 60 words.
4. Focus on checking their time complexity, space complexity, logic flow, and edge cases.
5. If they get stuck, offer subtle hints instead of full code solutions.`;
    ...
    try {
        ...
        return { text: response.text, tokensRemaining: user.tokens };
    } catch (error) {
        console.error("Gemini AI interviewChat error:", error);
        throw new Error("Failed to generate AI response");
    }
};
