...
Focus only on programming and DSA topics.`
        ...
        socket.on("join-room", ({ room, userName: pName }) => {
            socket.join(room);
            socket.peerRoom = room;
            socket.peerUserName = pName;
        ...
        socket.on("peer-language-change", ({ room, language: lang }) => {
            socket.to(room).emit("peer-language-change", { language: lang });
        });

        socket.on("chat-message", ({ room, userName: name, message, time }) => {
            socket.to(room).emit("chat-message", { userName: name, message, time });
        });

        socket.on("problem-selected", ({ room, problem }) => {
            socket.to(room).emit("problem-selected", { problem });
        });

        socket.on("draw-line", ({ room, x0, y0, x1, y1, color, width }) => {
            socket.to(room).emit("draw-line", { x0, y0, x1, y1, color, width });
        });
        ...
        socket.on("end-peer-interview", ({ room }) => {
            socket.to(room).emit("end-peer-interview");
        });
